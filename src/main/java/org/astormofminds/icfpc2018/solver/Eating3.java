...
class Eating3 implements Solver {
    ...
    public List<Command> getCompleteTrace() {
        ...
        for (int i = 0; i < sweeps; i++) {
            eatAllLayers(zmin, ymax, zmax, numBots);
            if (i + 1 < sweeps) {
                numBots = moveBotsToNewStart(zmin, ymax, numBots);
            }
        }
    ...
    private int moveBotsToNewStart(int zmin, int ymax, int numBots) {
        int newX = posx + ((numBots) * 3);
        while (newX + ((numBots - 1) * 3) > currentMatrix.getResolution() - 1) {
            fuseOneBot(numBots);
            numBots--;
        }
        while (posz < zmin - 1) {
        ...
        while (posx < newX) {
            int steps = Math.min(newX - posx, 15);
            result.add(Command.sMove(Difference.of(steps, 0, 0)));
            posx += steps;
        }
        //move one lower up, because we eat from the inside
    ...
    private void eatAllLayers(int zmin, int ymax, int zmax, int numBots) {
    ...
    private void eatAll(int numBots, int zEat) {
        ...
        //optimize waits
        Optimizer.optimizeBotWaits(11 + extraSteps, numBots, result);
    }

    private int checkHarmonic(int numbots) {
        if (!hasEaten) return 0;
